...
void TM_LIS302DL_INT_ReadAxes(TM_LIS302DL_LIS3DSH_t* Axes_Data) {
  ...
  // Read data from accelerometer over SPI using TM_LIS302DL_LIS3DSH_INT_ReadSPI function 
  TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&d[0], LIS302DL_OUT_X_ADDR, count);
  TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&d[1], LIS302DL_OUT_Y_ADDR, count);
  TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&d[2], LIS302DL_OUT_Z_ADDR, count);
    
  // Put received data into Axes_Data 
  Axes_Data->X = (int16_t) (d[0]) * TM_LIS3DSH_INT_Sensitivity;
  Axes_Data->Y = (int16_t) (d[1]) * TM_LIS3DSH_INT_Sensitivity;
  Axes_Data->Z = (int16_t) (d[2]) * TM_LIS3DSH_INT_Sensitivity;    
  // Switch X and Y axes
  d_xy  = Axes_Data->X;
